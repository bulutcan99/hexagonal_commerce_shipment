...
func (us *UserService) ListUsersWithLimit(ctx context.Context, skip, limit uint64) ([]domain.User, *domain.Error) {
	var users []domain.User
	params := util.GenerateCacheKeyParams("users", skip, limit)
	cachedUsers, err := us.cache.Get(ctx, params)
	if err == nil {
		err := util.Deserialize(cachedUsers, &users)
		if err != nil {
	...
	users, errDomain := us.userRepo.GetAllUsersWithLimit(ctx, skip, limit)
	if errDomain != nil {
		return nil, &domain.Error{
			Code:    domain.ErrSqlSelect,
			Message: "failed to get users from database",
